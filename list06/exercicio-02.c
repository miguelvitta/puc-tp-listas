...
float** sumMatrices(float** firstMatrix, float** secondMatrix, int numRowsFirst,
...
int main() {
    printf(
        ...
        "  1. Dynamically creates a float matrix.\n"
        "  2. Reads values into the matrix from user input.\n"
        "  3. Computes the sum of two matrices.\n"
    ...
    sumMatrix = sumMatrices(firstMatrix, secondMatrix, numRowsFirst,
    ...
    freeMatrix(firstMatrix, numRowsFirst);
    freeMatrix(secondMatrix, numRowsSecond);
    freeMatrix(sumMatrix, numRowsFirst);
    return 0;
}
