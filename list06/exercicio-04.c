...
float** multiplyMatrices(float** firstMatrix, float** secondMatrix,
                         int numRowsFirst, int numColsFirst, int numRowsSecond,
                         int numColsSecond);
...
int main() {
    printf(
        ...
        "  3. Computes the multiplication of two matrices.\n"
    ...
    resultMatrix = multiplyMatrices(firstMatrix, secondMatrix, numRowsFirst,
                                    numColsFirst, numRowsSecond, numColsSecond);
    if (resultMatrix == NULL) {
        fprintf(
            stderr,
            "Error - The number of columns in the first matrix must match the "
    ...
    printMatrix(resultMatrix, numRowsFirst, numColsSecond);
...
float** multiplyMatrices(float** firstMatrix, float** secondMatrix,
    ...
    int numRowsResult = numRowsFirst;
    int numColsResult = numColsSecond;

    float** matrix = NULL;
    matrix = malloc(sizeof(float*) * numRowsResult);
    for (int i = 0; i < numRowsResult; i++) {
        matrix[i] = calloc(numColsResult, sizeof(float));
    }

    for (int i = 0; i < numRowsResult; i++) {
        for (int j = 0; j < numColsResult; j++) {
            for (int k = 0; k < numColsFirst; k++) {
